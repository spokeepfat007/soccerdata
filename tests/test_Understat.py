...
@pytest.mark.asyncio
async def test_read_player_season_stats(understat_epl_1516: Understat) -> None:
    player_season_stats = await understat_epl_1516.read_player_season_stats()
    assert isinstance(player_season_stats, pd.DataFrame)
    assert len(player_season_stats) == 550

@pytest.mark.asyncio
async def test_read_player_match_stats(understat_epl_1516: Understat) -> None:
    player_match_stats = await understat_epl_1516.read_player_match_stats()
    assert isinstance(player_match_stats, pd.DataFrame)

@pytest.mark.asyncio
async def test_read_player_match_stats_new_columns(understat_epl_1516: Understat) -> None:
    player_match_stats = await understat_epl_1516.read_player_match_stats()
...
@pytest.mark.asyncio
async def test_read_shots(understat_epl_1516: Understat) -> None:
    shots_all = await understat_epl_1516.read_shot_events()
    assert isinstance(shots_all, pd.DataFrame)
    assert len(shots_all) == 9_819
    shots_utd_bou = await understat_epl_1516.read_shot_events(460)
    ...
    with pytest.raises(
        ...
        await understat_epl_1516.read_shot_events(42)
